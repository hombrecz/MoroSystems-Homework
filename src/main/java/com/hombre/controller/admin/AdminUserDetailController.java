...
public class AdminUserDetailController {
	...
	public void initBinder(WebDataBinder dataBinder) {
		...
		dataBinder.registerCustomEditor(Account.class, new AccountEditor(
				accountBo));
	}

	@RequestMapping(value = "admin/adminUserDetail", method = RequestMethod.GET)
	public ModelAndView listDetails(@ModelAttribute("user") User user,
	...
	public String deleteBook(@ModelAttribute("user") User user,
			@ModelAttribute("book") Book book,
			@ModelAttribute("account") Account account) {
		bookBo.delete(book);
		return "redirect:/admin/adminUserDetail?userid="
				+ book.getUser().getUserid() + "&username="
				+ book.getUser().getUsername() + "&enabled="
				+ book.getUser().getEnabled();
	}
	...
	public String deleteAccount(@ModelAttribute("user") User user,
		...
		return "redirect:/admin/adminUserDetail?userid="
				+ account.getUser().getUserid() + "&username="
				+ account.getUser().getUsername() + "&enabled="
				+ account.getUser().getEnabled();
	}
	...
	public String onSubmit(@ModelAttribute("account") Account account,
	...
	public String favoriteBook(@ModelAttribute("user") User user) {
		...
		return "redirect:/admin/adminUserDetail?userid="
				+ updated_user.getUserid() + "&username="
				+ updated_user.getUsername() + "&enabled="
				+ updated_user.getEnabled();
	}
	...
	public String addAccount(@ModelAttribute("user") User user) {
		...
		return "redirect:/admin/adminUserDetailAddEditAccount?id="
				+ help
				+ "&name=Account name&accountPrefix=123456&accountNumber=1234567890&bankCode=1234&user.userid="
				+ user.getUserid();
	}
